...
async function readWhatsAppFile(file) {
  ...
  if (lower.endsWith(".zip")) {
    ...
    if (!txtFiles.length) {
      throw new Error("El ZIP no contiene ningún archivo TXT.");
    }
    ...
    if (!chatText) {
      throw new Error("No se encontró un archivo de chat de WhatsApp válido dentro del ZIP.");
    }

    return chatText;
  }

  if (lower.endsWith(".txt") || file.type === "text/plain" || !file.type) {
    return file.text();
  }

  throw new Error("Formato no compatible. Selecciona un archivo .txt o .zip.");
}
...
/* =========================================================
   FUNCIONES EXTRA: WRAPPED / TELEMETRÍA / ANUNCIOS
========================================================= */

function buildTelemetryStats(messages, analysis) {
  const { users, stats } = analysis;
  const totalWords = users.reduce((n, u) => n + (stats[u]?.words || 0), 0);
  const names = calculateNames(messages, users).slice(0, 20);
  return {
    totalMessages: messages.length,
    totalWords,
    topNames: names,
    topWords: users.flatMap((u) => (stats[u]?.top || []).map(([word, count]) => ({ user: u, word, count }))).sort((a,b) => b.count-a.count).slice(0, 30),
    emojis: users.reduce((n,u) => n + (stats[u]?.emojiCount || 0), 0),
    parserRecognized: messages.length,
  };
}

function buildWrapped(messages, users, stats) {
  const totalWords = users.reduce((n, u) => n + (stats[u]?.words || 0), 0);
  const totalEmojis = users.reduce((n, u) => n + (stats[u]?.emojiCount || 0), 0);
  const laughs = users.reduce((n, u) => n + (stats[u]?.laughCount || 0), 0);
  const love = users.reduce((n, u) => n + (stats[u]?.loveCount || 0), 0);
  const night = messages.filter((m) => m.date.getHours() >= 22 || m.date.getHours() < 6).length;
  const nightPct = messages.length ? Math.round((night / messages.length) * 100) : 0;
  const winner = [...users].sort((a, b) => (stats[b]?.messages || 0) - (stats[a]?.messages || 0))[0] || "—";
  const avg = messages.length ? Math.round(totalWords / messages.length) : 0;

  let category = "EL ANALISTA DEL CHAT";
  if (nightPct >= 60) category = "EL NOCTÁMBULO DEL CHAT";
  else if (laughs > messages.length * 0.08) category = "EL CAÓTICO GRACIOSO";
  else if (love > messages.length * 0.04) category = "EL CARIÑOSO";
  else if (avg >= 35) category = "EL LADRILLO HUMANO";
  else if (avg <= 7) category = "EL REY DEL MENSAJE CORTO";

  return { totalMessages: messages.length, totalWords, totalEmojis, laughs, love, nightPct, winner, avgWords: avg, category };
}
...
async function sendTelemetry(payload) {
  try {
    ...
    // La telemetría nunca debe romper el análisis.
  }
}

function FunStats({ messages, users, stats, setSearch }) {
  const fun = useMemo(() => {
    const by = (key) => [...users].sort((a,b) => (stats[b]?.[key] || 0) - (stats[a]?.[key] || 0))[0] || "—";
    return {
      funny: by("laughCount"), loving: by("loveCount"), swearing: by("swearCount"),
      thanks: by("thanksCount"), sorry: by("sorryCount"), messages: by("messages"),
    };
  }, [users, stats]);

  return (
    <section className="panel fun-panel">
      <div className="section-title-row"><div><h3>🏆 Premios del chat</h3><p className="chart-desc">Resultados calculados a partir de las estadísticas reales.</p></div><Trophy size={22}/></div>
      <div className="award-grid">
        <div><b>😂 Más gracioso</b><span>{fun.funny}</span></div>
        <div><b>❤️ Más cariñoso</b><span>{fun.loving}</span></div>
        <div><b>🤬 Más palabrotas</b><span>{fun.swearing}</span></div>
        <div><b>🙏 Más gracias</b><span>{fun.thanks}</span></div>
        <div><b>😔 Más perdones</b><span>{fun.sorry}</span></div>
        <div><b>💬 Más mensajes</b><span>{fun.messages}</span></div>
      </div>
    </section>
  );
}

function WrappedPage({ messages, users, stats }) {
  const wrapped = useMemo(() => buildWrapped(messages, users, stats), [messages, users, stats]);

  const download = () => {
    downloadText("whatsapp-wrapped.txt", [
      "WHATSAPP WRAPPED", "", `Mensajes: ${wrapped.totalMessages}`,
      `Palabras: ${wrapped.totalWords}`, `Emojis: ${wrapped.totalEmojis}`,
      `JAJA / risas detectadas: ${wrapped.laughs}`, `Expresiones cariñosas: ${wrapped.love}`,
      `Actividad nocturna: ${wrapped.nightPct}%`, `Media: ${wrapped.avgWords} palabras/mensaje`,
      `Más mensajes: ${wrapped.winner}`, `Categoría: ${wrapped.category}`,
    ].join("\n"));
  };

  return <main className="extra-page">
    <div className="extra-hero"><span className="eyebrow">WHATSAPP WRAPPED</span><h1>Tu chat en una historia.</h1><p>Un resumen divertido y compartible de vuestra conversación.</p></div>
    <section className="wrapped-card">
        <span>WHATSAPP WRAPPED</span><h2>{wrapped.category}</h2>
        <div className="wrapped-grid">
          <div><b>💬</b><strong>{wrapped.totalMessages.toLocaleString("es-ES")}</strong><span>mensajes</span></div>
          <div><b>🗣️</b><strong>{wrapped.avgWords}</strong><span>palabras/mensaje</span></div>
          <div><b>😂</b><strong>{wrapped.laughs.toLocaleString("es-ES")}</strong><span>risas</span></div>
          <div><b>❤️</b><strong>{wrapped.love.toLocaleString("es-ES")}</strong><span>expresiones cariñosas</span></div>
          <div><b>🌙</b><strong>{wrapped.nightPct}%</strong><span>actividad nocturna</span></div>
          <div><b>🏆</b><strong>{wrapped.winner}</strong><span>más mensajes</span></div>
        </div>
        <button className="ai-save-button" onClick={download}><Download size={16}/> Descargar Wrapped</button>
      </section>
  </main>;
}
...
function Dashboard({
  ...
  return (
    <main>
      ...
      <SectionTitle icon={Trophy} title="Estadísticas divertidas" description="Una capa de premios y resultados pensada para compartir." />
      <FunStats messages={messages} users={users} stats={stats} setSearch={setSearch} />
...
function AIReport({ report, onSave, onClear }) {
  ...
  return (
    <article className="ai-report">
      ...
      <div className="ai-two-column">
        <Section icon="🗣️" title="Estilo de comunicación">
          <p>{report.communication_style || "No disponible."}</p>
        </Section>
        <Section icon="🤝" title="Compatibilidad comunicativa">
          <p>{report.compatibility || "No hay suficientes datos para valorarla."}</p>
        </Section>
      </div>

      {report.themes?.length ? (
        <Section icon="🧩" title="Temas recurrentes"><List items={report.themes} /></Section>
      ) : null}

      {report.evolution ? (
        <Section icon="📈" title="Evolución"><p>{report.evolution}</p></Section>
      ) : null}

      {report.roast ? (
        <Section icon="🔥" title="Roast del chat"><p>{report.roast}</p></Section>
      ) : null}
...
Usa exactamente esta estructura:
{
  ...
  "communication_style": "descripción del estilo de comunicación",
  "compatibility": "compatibilidad comunicativa prudente",
  "themes": ["tema recurrente"],
  "evolution": "cómo cambia la conversación a lo largo del tiempo si hay datos suficientes",
  "roast": "roast humorístico, sin insultos degradantes"
}
...
`;
      ...
      sendTelemetry({ type: "ai", users, statistics, report: parsed }).catch(() => {});
    } catch (err) {
  ...
  const saveReport = () => {
    ...
    const text = [
      ...
      "",
      "COMPATIBILIDAD COMUNICATIVA",
      report.compatibility || "",
      "",
      "TEMAS RECURRENTES",
      ...(report.themes || []).map((x) => `• ${x}`),
      "",
      "EVOLUCIÓN",
      report.evolution || "",
      "",
    ...
    <main className="ai-page">
      ...
      <section className="panel privacy-panel">
        ...
        <div className="privacy-grid">
          ...
          <div><b>📊 Mejora anónima</b><span>Se envían estadísticas agregadas y resultados de IA al sistema de mejora; nunca el TXT completo desde esta función.</span></div>
        </div>
      </section>
    </main>
  );
    ...
    <div className="tutorial-backdrop" role="dialog" aria-modal="true" aria-label="Cómo exportar un chat">
      <div className="tutorial-modal">
        ...
        <div className="tutorial-body">
          ...
          <div className="tutorial-step"><b>4</b><div><strong>Sube el archivo</strong><p>Puedes subir el <code>.txt</code> o el <code>.zip</code> generado por WhatsApp. El analizador busca automáticamente el TXT del chat dentro del ZIP, sin importar cómo se llame.</p></div></div>
...
function AdminPage() {
  ...
  const load = async () => {
    setError("");
    try {
      const response = await fetch("/api/admin/config", { headers: { "X-Admin-Token": token } });
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || "No autorizado.");
      setConfig(data.config);
    } catch (e) { setError(e?.message || "No se pudo cargar el panel."); }
  };
  return <main className="admin-page">
    <div className="admin-hero"><span className="eyebrow">ADMINISTRACIÓN</span><h1>Centro de control</h1><p>Gestiona la configuración que también puedes modificar desde Telegram.</p></div>
    {!config ? <section className="panel admin-login"><Lock size={24}/><h3>Acceso privado</h3><p>Introduce el token del panel configurado en Cloudflare.</p><input type="password" value={token} onChange={e => setToken(e.target.value)} placeholder="ADMIN_PANEL_TOKEN"/><button className="ai-main-button" onClick={load}><Bot size={17}/> Entrar</button>{error && <div className="warning">{error}</div>}</section> :
      <section className="admin-grid">{Object.entries(config).map(([category, values]) => <section className="panel" key={category}><div className="section-title-row"><h3>{category}</h3><span>{values.length}</span></div><div className="admin-tags">{values.slice(0, 200).map(v => <span key={`${category}-${v}`}>{v}</span>)}</div></section>)}</section>}
  </main>;
}
...
function App() {
  ...
  const handleFile = async (
    ...
    try {
      ...
      setMessages(parsed);
      setRawChat(text);
      const quick = calculateStats(parsed);
      sendTelemetry({
        type: "chat",
        platform: "whatsapp",
        fileType: /\.zip$/i.test(file.name) ? "zip" : "txt",
        messages: parsed.length,
        users: Object.keys(quick.stats || {}).length,
        stats: buildTelemetryStats(parsed, quick),
      }).catch(() => {});
        ...
        <main>
          ...
          <div className="upload-page">
            ...
            <label className="upload-button">
              ...
              Seleccionar chat TXT o ZIP o ZIP
    ...
    <div className="app wa-app">
      <aside className="wa-sidebar">
        ...
        <div className="wa-chat-list">
          ...
          <button className={`wa-nav-item ${page === "wrapped" ? "active" : ""}`} onClick={() => { setPage("wrapped"); setSearch(null); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
            <span className="wa-nav-icon"><Trophy size={20} /></span>
            <span className="wa-nav-copy"><strong>WhatsApp Wrapped</strong><small>Resumen compartible</small></span>
          </button>
        </div>
      ...
      <section className="wa-content">
        ...
        {!search && page === "wrapped" && (
          <WrappedPage messages={messages} users={analysis.users} stats={analysis.stats} />
        )}
      </section>
    </div>
  );
...
function Root() {
  return window.location.pathname === "/admin" ? <AdminPage /> : <App />;
}
